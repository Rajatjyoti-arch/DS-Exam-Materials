...
> **Course Code:** BCSE2C05 / BCCS2C01 / BMNC2C01 | **Semester:** II
> This material covers every concept from the syllabus in simple language so you can solve all question papers confidently.
...
- **Symmetric Binary:** Both values are equally important.
  ...
  - **Formula:** Simple Matching Coefficient = (f₁₁ + f₀₀) / (f₁₁ + f₁₀ + f₀₁ + f₀₀)
...
- **Asymmetric Binary:** One value (usually 1) is more important/rare.
  ...
  - **Formula:** Jaccard Coefficient = f₁₁ / (f₁₁ + f₁₀ + f₀₁)
...
**Step 3:** Variance = 14.80 / 5 = **2.96**

**Step 4:** Standard Deviation = √2.96 = **1.72**
...
| :--- | :--- | :--- | :--- |
| **Delete rows** | Remove entire row with missing value | Missing % is very small (<5%) | `df.dropna()` |
| **Delete columns** | Remove entire column | Column has >50% missing | `df.drop(columns=['col'])` |
| **Fill with Mean** | Replace NaN with column average | Numeric, normally distributed | `df['col'].fillna(df['col'].mean())` |
| **Fill with Median** | Replace NaN with column median | Numeric, skewed data or outliers | `df['col'].fillna(df['col'].median())` |
| **Fill with Mode** | Replace NaN with most frequent value | Categorical data | `df['col'].fillna(df['col'].mode()[0])` |
| **Forward Fill (ffill)** | Use previous valid value | Time-series data | `df['col'].ffill()` |
| **Backward Fill (bfill)** | Use next valid value | Time-series data | `df['col'].bfill()` |
| **Interpolation** | Estimate based on surrounding values | Continuous, smooth data | `df['col'].interpolate()` |
...
Z = (X − μ) / σ
```
Example: μ=30, σ=14.14, for X=45: (45−30)/14.14 = **1.06**
...
```
⚠️ Problem: Model might think Large (2) > Small (0). Use only for ordinal data.
...
- **Equal-Width:** Each bin covers same range. Width = (max−min)/num_bins.
...
Distances:
- d(P,A) = √((4−1)²+(5−2)²) = √(9+9) = √18 = **4.24**
- d(P,B) = √((4−2)²+(5−3)²) = √(4+4) = √8 = **2.83**
- d(P,C) = √((4−6)²+(5−7)²) = √(4+4) = √8 = **2.83**
- d(P,D) = √((4−7)²+(5−8)²) = √(9+9) = √18 = **4.24**
- d(P,E) = √((4−3)²+(5−3)²) = √(1+4) = √5 = **2.24**

3 nearest: E (Cat, 2.24), B (Cat, 2.83), C (Dog, 2.83)
Majority: 2 Cat, 1 Dog → **Predicted: Cat** ✅
...
3. **Assign** each data point to the nearest centroid → forms K clusters.
...
| Point | |d to C1=2.5| | |d to C2=9.0| | Cluster |
|:---:|:---:|:---:|:---:|
| 2 | 0.5 | 7.0 | C1 |
| 3 | 0.5 | 6.0 | C1 |
| 8 | 5.5 | 1.0 | C2 |
...
Same clusters as before → **Converged!** ✅
Final: Cluster 1 = {2, 3}, Cluster 2 = {8, 9, 10}
...
- Z = (X − μ) / σ
- If |Z| > 3 → Outlier (the point is 3+ standard deviations from mean)
...
| **F1-Score** | 2×(P×R)/(P+R) | Harmonic mean of precision & recall | Imbalanced datasets |
...
| **Confidence** | freq(A∩B) / freq(A) | Probability of B given A was bought |
| **Lift** | Confidence / Support(B) | Improvement over random chance |
...
Simple models (Linear Regression) = Easy to understand but less accurate.
Complex models (Deep Learning) = Highly accurate but "black box."

**When to choose simple:** Regulated industries (banking, healthcare), need to explain decisions.
**When complex is OK:** Image recognition, NLP, when accuracy gap is large.
**Tools for explaining complex models:** LIME, SHAP.
...
| Fill NaN mean | `df['col'].fillna(df['col'].mean())` |
| Drop NaN | `df.dropna()` |
...
| Histogram | `plt.hist(data, bins=15)` |
| Boxplot | `sns.boxplot(x='cat', y='num', data=df)` |
| Scatter | `plt.scatter(x, y)` |
| Heatmap | `sns.heatmap(df.corr(), annot=True)` |
| NumPy mean | `np.mean(arr)` |
| NumPy std | `np.std(arr)` |
